...
public class RenderUtils {
    ...
    public static void drawNeonBox3D(MatrixStack matrices, double x1, double y1, double z1, double x2, double y2, double z2, Color color, boolean fill, int fillAlpha) {
        ...
        if (fill && fillAlpha > 0) {
            ...
            BufferBuilder buffer = tessellator.getBuffer();
            buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
            buffer.vertex(matrix, (float) x1, (float) y1, (float) z1).color(fr, fg, fb, fa).next();
            buffer.vertex(matrix, (float) x2, (float) y1, (float) z1).color(fr, fg, fb, fa).next();
            buffer.vertex(matrix, (float) x2, (float) y1, (float) z2).color(fr, fg, fb, fa).next();
            buffer.vertex(matrix, (float) x1, (float) y1, (float) z2).color(fr, fg, fb, fa).next();

            buffer.vertex(matrix, (float) x1, (float) y2, (float) z1).color(fr, fg, fb, fa).next();
            buffer.vertex(matrix, (float) x1, (float) y2, (float) z2).color(fr, fg, fb, fa).next();
            buffer.vertex(matrix, (float) x2, (float) y2, (float) z2).color(fr, fg, fb, fa).next();
            buffer.vertex(matrix, (float) x2, (float) y2, (float) z1).color(fr, fg, fb, fa).next();
            tessellator.draw();
            RenderSystem.disableBlend();
        }
