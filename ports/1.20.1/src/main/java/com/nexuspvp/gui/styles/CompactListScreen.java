...
public class CompactListScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(textRenderer, "GLEBKA", winX + 10, winY + 9, accent);
        context.drawTextWithShadow(textRenderer, "LIST", winX + 54, winY + 9, 0xFFFFFFFF);
        ...
        for (int i = 0; i < cats.length; i++) {
            ...
            context.drawTextWithShadow(textRenderer, c, tx + (tabW - tw) / 2, ty + 4, active ? 0xFFFFFFFF : 0xFF949BA4);
        }
        ...
        if (filterCategory.equalsIgnoreCase("THEMES")) {
            ...
            for (int i = 0; i < filtered.size(); i++) {
                ...
                if (enabled) {
                    context.drawTextWithShadow(textRenderer, "\u2714", chkX + 2, chkY + 1, 0xFFFFFFFF);
                }

                // Module name
                context.drawTextWithShadow(textRenderer, m.getName(), winX + 28, ry + 7, enabled ? 0xFFFFFFFF : 0xFFDBDEE1);

                // Settings gear
                int gearX = winX + listW - 40;
                context.drawTextWithShadow(textRenderer, "\u2699", gearX, ry + 7, isSelected ? accent : 0xFF8A93A4);
                ...
                context.drawTextWithShadow(textRenderer, keyText, kx, ry + 7, isBindingThis ? accent : 0xFF949BA4);
            }
        }
        ...
        if (selectedModule != null) {
            ...
            // Drawer Header
            context.drawTextWithShadow(textRenderer, selectedModule.getName(), drawX + 8, drawY + 8, accent);
            context.drawTextWithShadow(textRenderer, "X", drawX + drawW - 14, drawY + 8, 0xFFED4245);
            ...
            if (settings.isEmpty()) {
                ...
                for (Setting<?> s : settings) {
                    if (s instanceof BooleanSetting) {
                        ...
                        context.drawTextWithShadow(textRenderer, s.getName() + ": " + ms.getValue(), drawX + 10, curY + 7, accent);
                        curY += 26;
    ...
    private void renderStyleSelector(MatrixStack matrices, int x, int y, int w, int mouseX, int mouseY) {
        context.drawTextWithShadow(textRenderer, "SELECT GUI STYLE:", x + 4, y + 4, 0xFFFFFFFF);
        ...
        for (int i = 0; i < styles.length; i++) {
            ...
            context.drawTextWithShadow(textRenderer, s.getIcon() + " " + s.getDisplayName() + " - " + s.getDescription(), x + 8, by + 12, active ? 0xFFFFFFFF : 0xFFD8DEE9);
        }
    }
