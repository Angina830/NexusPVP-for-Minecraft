        ...
        synchronized (activeZones) {
            for (StunZone zone : activeZones) {
                ...
                if (!curStyle.equals("SquareOutline")) {
                    buffer.vertex(mat, x1, y2, z1).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x2, y2, z1).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x2, y2, z1).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x2, y2, z2).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x2, y2, z2).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x1, y2, z2).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x1, y2, z2).color(r, g, b, a * 0.6f).next();
                    buffer.vertex(mat, x1, y2, z1).color(r, g, b, a * 0.6f).next();
