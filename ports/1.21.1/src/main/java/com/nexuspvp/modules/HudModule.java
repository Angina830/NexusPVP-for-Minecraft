...
public class HudModule extends Module {
    ...
    public void onRender2D(MatrixStack matrices, float tickDelta) {
        ...
        int accent = rainbow.isEnabled() ? NexusPVP.getInstance().getThemeManager().getRainbowColor(0) : color.getColor().getRGB();
        ...
        if (arrayList.isEnabled()) {
            ...
            for (int i = 0; i < active.size(); i++) {
                ...
                int modColor = rainbow.isEnabled() ? NexusPVP.getInstance().getThemeManager().getRainbowColor(i * 100) : accent;
